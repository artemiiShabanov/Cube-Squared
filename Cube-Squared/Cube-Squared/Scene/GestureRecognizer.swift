...
final class GameGestureRecognizer {
    ...
    func touchMoved(_ touch: UITouch) {
        ...
        case .registering(let previousPoint, let previousDirs):
            var dirs: [(Direction, CGFloat)] = []
            let horizontalShift = abs(previousPoint.x - location.x)
            let verticalShift = abs(previousPoint.y - location.y)
            if horizontalShift > directionTreshold {
                dirs.append((location.x > previousPoint.x ? .right : .left, horizontalShift))
            }
            if verticalShift > directionTreshold {
                dirs.append((location.y > previousPoint.y ? .top : .bottom, verticalShift))
            }
            let sortedDirs = dirs.sorted(by: { $0.1 > $1.1 }).map { $0.0 }
            if !sortedDirs.isEmpty {
                if previousDirs != sortedDirs {
